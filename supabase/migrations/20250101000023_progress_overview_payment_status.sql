-- Surfaces payment_status in get_progress_overview so the dashboard/week
-- pages can tell "locked because you haven't finished the prior week"
-- apart from "locked because payment is required" instead of showing the
-- same generic locked message for both.
...
begin
  ...
  select jsonb_build_object(
    'enrolled', true,
    'track_id', v_track_id,
    'payment_status', (select payment_status from profiles where id = v_user_id),
    'overall', (
      select jsonb_build_object(
