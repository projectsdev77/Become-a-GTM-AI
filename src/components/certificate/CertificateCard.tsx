...
export default function CertificateCard({ fields }: { fields: CertificateFields }) {
  const accent = fields.accent_color || 'var(--color-signal)'
  return (
    <div className="mx-auto max-w-2xl overflow-hidden rounded-panel border border-stone bg-paper" style={{ boxShadow: 'var(--shadow-app)' }}>
      <div className="flex items-center justify-between bg-ink px-7 py-4">
        <div className="flex items-center gap-2.5">
          <Monogram size={24} reverse />
          {fields.logo_url ? (
            <img src={fields.logo_url} alt="" className="h-6 object-contain" />
          ) : (
            <span className="text-[13px] font-semibold text-paper">Become a GTM AI</span>
          )}
        </div>
        <span className="meta text-paper/50">Certificate</span>
      </div>
      ...
      <div className="px-8 py-10 text-center sm:px-12">
        ...
        <p className="mx-auto mt-7 max-w-lg text-[17px] leading-relaxed text-body">{fields.body_text}</p>

        {(fields.signature_name || fields.signature_title) && (
          <div className="mx-auto mt-10 inline-block border-t border-stone-strong pt-2 text-left">
            {fields.signature_name && <p className="font-semibold text-ink">{fields.signature_name}</p>}
            {fields.signature_title && <p className="meta">{fields.signature_title}</p>}
          </div>
        )}
      </div>
    </div>
